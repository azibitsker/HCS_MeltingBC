...
void massBalance::solveMassBal(chemkin SR,double rho_w,vector<double> yk_f, vector<double> x)
{
	...
	for (int i = 0; i < SR.Ns; i++) {
		...
		yk_gh[i] = 1 / (1 + SR.mdot_w * dx / (2 * rho_w * Ds)) * (yk_f[i] * (1 - SR.mdot_w * dx / (2 * rho_w * Ds)) + (SR.mdot_k[i] * dx) / (rho_w * Ds));
		y_sum += yk_gh[i];

		yk_w[i] = (yk_f[i] + yk_gh[i]) / 2; // update species mass fractions at the boundary
